...
## 2026-08-26: `oteltest` pins the global OTel defaults once, rather than trying to restore them

`oteltest.NewSpanRecorder` set the global `TracerProvider` for a test and restored `otel.GetTracerProvider()`'s prior value in cleanup. That "save, then restore" pattern is unsafe against `go.opentelemetry.io/otel`'s actual global state: its defaults for both the tracer provider and the text map propagator are placeholders that, once any concrete value is ever set, wire their delegate to it permanently for the life of the process — an internal detail of that module, not a documented guarantee of `otel.SetTracerProvider`/`otel.SetTextMapPropagator`, but observably true of the version this module depends on. If a helper's "previous" ever turned out to be that raw placeholder (i.e. the helper ran first in a test binary), restoring it would not undo the installation; it would leave the placeholder permanently forwarding to whatever the helper had installed.

We had already hit this for the propagator (`http/otel_test.go`'s `usePropagators`, from the issue-197 work): no shutdown mechanism exists to neutralize a leaked propagator, so that helper always left a known-inert composite behind in cleanup rather than trying to restore "previous". A follow-up review of `oteltest.NewSpanRecorder` found the identical pattern for the tracer provider, unfixed. A standalone repro against the real SDK showed the tracer-provider case was, in practice, self-neutralizing: `sdktrace.TracerProvider.Shutdown()` makes every later `.Tracer()` call on that provider return a no-op, checked live on every call — so a permanently-wired-but-shut-down delegate behaves like the safe case anyway, provided cleanup always shuts the provider down before restoring. But that safety was an accident of call order in one helper, not a property of the pattern, and the ask was to fix it at the root rather than re-derive the same patch per call site.

Alternatives considered:

- Leave `NewSpanRecorder`'s restore-previous as is, since it was empirically safe: rejected, because the safety depended on an implementation detail (`Shutdown()`'s neutering) holding at every call site forever, which is exactly the kind of unstated invariant this whole investigation exists to stop relying on.
- Keep each helper unconditionally leaving a fresh inert stand-in behind in cleanup, as `usePropagators` originally did, rather than restoring "previous" at all: rejected for `oteltest`, because that breaks the *nested* case — an inner `NewSpanRecorder`/propagator helper has to hand control back to an outer one that is still active and still expected to keep recording, not to a fresh no-op.
- A package-level `sync.Once` that pins both globals to inert stand-ins (`noop.NewTracerProvider()`, an empty composite propagator) before anything ever saves "previous": chosen.

Decision: `oteltest` now has an unexported `pinGlobals`, a `sync.OnceFunc` that installs the inert stand-ins once, called at the top of both `NewSpanRecorder` and a new exported `UsePropagators`, before either reads the current global to save as "previous". Because the pin always runs first, "previous" is never the SDK's raw placeholder — it is always either the pin itself or a concrete value an earlier helper call installed — so plain save-then-restore is correct in every case, including nested ones, without depending on `Shutdown()` behavior or any other implementation detail of the SDK. `usePropagators` in `http/otel_test.go` was migrated onto `oteltest.UsePropagators`, and `jobs/runner_test.go`'s hand-rolled top-level tracer provider and propagator were replaced with calls to the shared helpers, so the fix now lives in one place instead of being re-derived at each call site.
